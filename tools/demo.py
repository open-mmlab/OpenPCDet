    ...
    with torch.no_grad():
        for idx, data_dict in enumerate(demo_dataset):
            ...
            V.draw_scenes(
                ...
                ref_boxes=pred_dicts[0]['pred_boxes'][:len(data_dict.get('gt_boxes', None)[0])],
                ref_scores=pred_dicts[0]['pred_scores'][:len(data_dict.get('gt_boxes', None)[0])],
                ref_labels=pred_dicts[0]['pred_labels'][:len(data_dict.get('gt_boxes', None)[0])]
            )
            mlab.show(stop=True)

...
class VoxelRCNNRelationHead(RoIHeadTemplate):
    def __init__(self, backbone_channels, model_cfg, point_cloud_range, voxel_size, num_class=1, object_relation_config=None, **kwargs):
        ...
        for k in range(0, self.model_cfg.CLS_FC.__len__()):
            cls_fc_list.extend([
                nn.Linear(self.head_input_channels if k==0 else pre_channel, self.model_cfg.CLS_FC[k], bias=False),
        ...
        for k in range(0, self.model_cfg.REG_FC.__len__()):
            reg_fc_list.extend([
                nn.Linear(self.head_input_channels if k==0 else pre_channel, self.model_cfg.REG_FC[k], bias=False),
    ...
    def forward(self, batch_dict):
        ...
        batch_dict['pooled_features'] = pooled_features.view(B, N, self.model_cfg.SHARED_FC[-1])

...
class CenterHeadGroup(nn.Module):
    def __init__(self, model_cfg, input_channels, num_class, class_names, grid_size, point_cloud_range, voxel_size,
        ...
        self.shared_conv = torch.nn.Sequential(
                nn.Conv2d(input_channels, self.model_cfg.SHARED_CONV_CHANNEL, 
        ...
        for k in range(self.model_cfg.NUM_HM_CONV - 1):
            hm_list.append(nn.Sequential(
                nn.Conv2d(inp_channels, outp_channels, kernel_size=3,
                    stride=1, padding=1, groups=groups, bias=use_bias),
                nn.GroupNorm(num_heads, outp_channels),
                nn.ReLU()))
            if k == 0:
        ...
        hm_list.append(nn.Conv2d(outp_channels, hm_total_outp_channels, kernel_size=3, 
            stride=1, padding=1, groups=groups, bias=True))
        self.heatmap_convs = nn.Sequential(*hm_list)
        self.heatmap_convs[-1].bias.data.fill_(-2.19)
        ...
        for k in range(num_convs - 1):
            attr_list.append(nn.Sequential(
                nn.Conv2d(inp_channels, outp_channels, kernel_size=3,
                    stride=1, padding=1, groups=groups, bias=use_bias),
                nn.GroupNorm(num_heads * len(head_dict), outp_channels),
                nn.ReLU()))
            if k == 0:
                inp_channels = outp_channels
        ...
        attr_list.append(nn.Conv2d(outp_channels, attr_total_outp_channels, kernel_size=3, 
            stride=1, padding=1, groups=groups, bias=True))
    ...
    def forward(self, data_dict):
        ...
        #t1=time.time()
        if not spatial_features_2d.is_contiguous(memory_format=torch.contiguous_format):
            spatial_features_2d = spatial_features_2d.to(memory_format=torch.contiguous_format)

        x = self.shared_conv(spatial_features_2d)

        # Run heatmap convolutions and gather the actual channels
        heatmaps = self.heatmap_convs(x)

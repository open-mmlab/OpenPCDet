...
class SeparateHead(nn.Module):
    def __init__(self, input_channels, sep_head_dict, init_bias=-2.19, use_bias=False, 
        ...
        for cur_name in self.sep_head_dict:
            ...
            self.num_conv_per_branch.append(num_conv)

            fc_list = []
            for k in range(num_conv - 1):
                fc_list.append(nn.Sequential(
                    nn.Conv2d(input_channels, input_channels, kernel_size=self.kernel_size,
                        stride=1, padding=0, bias=use_bias),
                    nn.BatchNorm2d(input_channels),
                    nn.ReLU()
                ))
            fc_list.append(nn.Conv2d(input_channels, output_channels, kernel_size=self.kernel_size,
        ...
        assert all((nc0 == nc for nc in self.num_conv_per_branch))
        self.num_conv_per_branch = nc0
        self.slice_size = self.kernel_size + self.num_conv_per_branch
